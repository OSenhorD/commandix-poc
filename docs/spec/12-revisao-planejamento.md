...
|------|-------------|-------------|
| Módulos NestJS | [03-arquitetura](./03-arquitetura.md): inclui `users/` (opcional) | `AGENTS.md`: **sem** `users/` — bootstrap é a única criação de usuários |
...
- [x] Truncamento de `responseBody` (10 240 bytes — [05-api §5.4](./05-api.md#truncamento-de-responsebody))
- [x] PATCH parcial integrações ([05-api §5.3](./05-api.md#patch-integrationsid))
...
| 6 | ~~Completar [05-api](./05-api.md): PATCH~~ — resolvido | — |
...
| Truncamento | 10 240 bytes UTF-8 em `responseBody`; sufixo se cortado |
| PATCH integração | Parcial; todos opcionais; `authKey` omitido = mantém |
